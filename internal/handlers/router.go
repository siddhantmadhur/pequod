...
func RegisterRoutes(cfg RouteConfig) {
	...
	e.GET("/media/:mediaId/streams/:sessionId/master.m3u8", cfg.MediaHandler.GetMasterPlaylist, authMw.RequireStreamAuth)
	e.GET("/media/:mediaId/streams/:sessionId/:segment/stream.ts", cfg.MediaHandler.GetStreamFile, authMw.RequireStreamAuth)
	e.GET("/media/:mediaId/direct/:fileName", cfg.MediaHandler.GetDirectPlayVideo, authMw.RequireStreamAuth)
	e.GET("/server/streaming/sessions", cfg.MediaHandler.GetAllSessions, authMw.RequireAdmin)
}
