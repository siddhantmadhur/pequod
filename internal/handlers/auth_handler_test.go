...
type mockAuthServiceForHandler struct {
	...
	validateFunc     func(tokenString string) (*domain.User, error)
...
func (m *mockAuthServiceForHandler) ValidateToken(tokenString string) (*domain.User, error) {
	if m.validateFunc != nil {
		return m.validateFunc(tokenString)
	}
	return &domain.User{UID: 1, Username: "admin", PermissionLevel: 0}, nil
}
