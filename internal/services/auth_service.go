...
func (s *authService) RefreshToken(ctx context.Context, refreshTokenString string) (*domain.TokenPair, error) {
	...
	// Graceful rotation: persist new session first. Only delete old session after creation succeeds
...
func (s *authService) CreateUser(ctx context.Context, currentUser *domain.User, username, password string, permLevel int64, isWizardMode bool) error {
	...
	if isWizardMode {
		...
		return s.userRepo.Create(ctx, username, hashedPassword, domain.RoleAdmin)
	}

	if currentUser == nil {
		return domain.ErrUnauthorized
	}
	if currentUser.PermissionLevel != domain.RoleAdmin {
...
func (s *authService) ValidateToken(ctx context.Context, tokenString string) (*domain.User, error) {
	...
	// SEC-01 / SEC-03: Verify that the session is still active in the database
	// This closes the stateless access token gap, guaranteeing immediate revocation on logout
	activeSession, err := s.sessionRepo.GetByID(ctx, sid)
	if err != nil || activeSession == nil {
		return nil, domain.ErrUnauthorized
	}

	if time.Now().After(activeSession.AccessExpiresAt) {
		return nil, domain.ErrUnauthorized
	}
