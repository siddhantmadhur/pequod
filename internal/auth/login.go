...
func generateToken(user *database.Profile, secret string) (string, string, error) {
...
func storeToken(userId int64, accessToken string, refreshToken string, device string, deviceName string, clientName string, clientVersion string, query *database.Queries) (database.Session, error) {
	return query.CreateSession(context.Background(), database.CreateSessionParams{
		ID:               uuid.NewString(),
		UserID:           userId,
		CreatedAt:        time.Now(),
...
func comparePasswords(query *database.Queries, username string, password string) (*database.Profile, error) {
